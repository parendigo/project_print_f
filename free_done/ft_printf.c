...
int				find_operator(t_tab *x)
{
	...
	while (x->str[x->i + j])
	{
		...
		{
			// ft_strdel(&x->allforms[x->co]);
			// ft_strdel(&x->allforms[x->co + 1]);
...
//	 ft_printf("%r\n", str);
//	 free(str);
	// ft_printf("%-10d, %+10i, %#10i, %010i, % 10i", i, i, i, i, i);
//	 ft_printf("%10d, %.10i, %0o, %+u, % x, %X, %-10s, %c, %s, %f", 123, 123, 123, 123, 123, 123, "Kolo", '\0', "56", 0.00006);
	// ft_printf("%-10s, %c, %s, %f", "Kolo", '\0', "56", 0.00006);
	// ft_printf("%.10s", NULL);
    printf("%.45f\n", 0.0000000000000001);
	ft_printf("%.45f", 0.0000000000000001);
