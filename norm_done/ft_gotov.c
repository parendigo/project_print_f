...
void	ft_gotov(va_list factor, t_tab *x)
{
	...
	|| x->cf == 'd' || x->cf == 'u' || x->cf == 'o' || x->cf == 'b')
		...
		x->allargs[x->co] = ft_strrejoin("0x",
				ft_itoa_b(va_arg(factor, long int), base, x, 'l'), 2);
...
void	ft_str_number(va_list factor, t_tab *x)
{
	...
	if (form_ll(x->allforms[x->co]) == 1)
		x->allargs[x->co] = ft_itoa_b(va_arg(factor, long long int),
				base, x, 'l');
	else if (form_ll(x->allforms[x->co]) == 2)
		x->allargs[x->co] = ft_itoa_b(va_arg(factor, long int),
				base, x, 'l');
	else if (form_hh(x->allforms[x->co]) == 1)
		x->allargs[x->co] = ft_itoa_b((signed char)va_arg(factor, int),
				base, x, 'h');
	else if (form_hh(x->allforms[x->co]) == 2)
		x->allargs[x->co] = ft_itoa_b((short int)va_arg(factor, int),
				base, x, 's');
	else
